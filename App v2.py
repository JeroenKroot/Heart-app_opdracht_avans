...
# 5) Map terug naar originele kolomnaam (ROBUSTER gemaakt voor ST_Slope)
    orig_cols = [c.strip() for c in X_one_row.columns]  # <-- FIX: strip spaties

    def norm(s: str) -> str:
        # <-- FIX: normaliseer zodat ST_Slope, st_slope, "ST_Slope " allemaal matchen
        return str(s).strip().lower()

    orig_cols_norm = {norm(c): c for c in orig_cols}  # map genormaliseerd -> originele naam

    def to_base_feature(name: str) -> str:
        after = name.split("__", 1)[-1]   # bv "ST_Slope_Up" of "Age"
        after_n = norm(after)

        # 5a) Exact match (numeriek kolommen)
        if after_n in orig_cols_norm:
            return orig_cols_norm[after_n]

        # 5b) One-hot match: pak de LANGSTE originele kolomnaam die prefix is
        #     (werkt voor ST_Slope_* en ChestPainType_* etc.)
        candidates = []
        for c in orig_cols:
            c_n = norm(c)
            if after_n.startswith(c_n + "_"):
                candidates.append(c)

        if candidates:
            return max(candidates, key=len)  # <-- pakt "ST_Slope" i.p.v. "ST"

        # 5c) Fallback (zou nu zelden nog nodig zijn)
