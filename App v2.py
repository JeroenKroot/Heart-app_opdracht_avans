...
orig_cols = list(X_one_row.columns)

def to_base_feature(name: str) -> str:
    after = name.split("__", 1)[-1]  # bv "ST_Slope_Up" of "Age"

    # Exact match (numeriek)
    if after in orig_cols:
        return after

    # One-hot: zoek de langste originele kolomnaam die prefix is
    matches = [c for c in orig_cols if after.startswith(c + "_")]
    if matches:
        return max(matches, key=len)  # pakt "ST_Slope" i.p.v. "ST"
